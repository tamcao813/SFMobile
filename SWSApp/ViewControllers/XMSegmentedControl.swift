        ...
        func addSegments(startingPosition starting: CGFloat, sections: Int, width: CGFloat, height: CGFloat) {
            for i in 0..<sections {
                ...
                case .hybridVertical:
                    ...
                    //let textSize = NSString(string: text).size(withAttributes: [NSAttributedStringKey.fontNSAttributedStringKey.font: halfSizeFont])

                    let spacing: CGFloat = 12
                    let imageHorizontalInset: CGFloat = (width - imageSize.width)/2

                    tab.imageEdgeInsets = UIEdgeInsetsMake(spacing, imageHorizontalInset, spacing + /*textSize.height*/ 5.0 + edgeHighlightHeight, imageHorizontalInset)
