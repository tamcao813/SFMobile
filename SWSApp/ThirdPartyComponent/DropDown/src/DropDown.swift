...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if  UserDefaults.standard.bool(forKey: "isBlackLineActive"){
            if(indexPath.row == 5){
                let lineView = UIView()
                let newFrame = CGRect(x:cell.frame.origin.x, y:cell.frame.size.height-1.0, width: cell.frame.size.width, height: 1.0)                
                lineView.frame = newFrame;
                lineView.backgroundColor = UIColor.black
                cell.addSubview(lineView)
                UserDefaults.standard.set(false, forKey: "isBlackLineActive")
            }
        }
